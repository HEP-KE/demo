...
def plot_power_spectra(k_theory, model_results, k_obs, Pk_obs, σPk_obs, 
    ...
    if k_obs is not None and Pk_obs is not None:
        plt.errorbar(k_obs, Pk_obs, yerr=σPk_obs, 
                    fmt='ko', markersize=5, 
    ...
    plt.ylabel('P(k) [(Mpc/h)³]', fontsize='x-large')
    plt.title('Matter Power Spectrum: Theory vs Observations', fontsize='x-large')
    plt.legend(loc='lower left', fontsize='x-large', framealpha=0.95, ncol=1)
    plt.xlim(1e-4, 20)
    plt.ylim(1e0, 2e5)
